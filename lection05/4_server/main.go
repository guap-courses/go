...
func AdvancedHandler(w http.ResponseWriter, r *http.Request) {
	...
	_, _ = w.Write([]byte("My name is " + r.URL.Query().Get("name")))
}

//  curl -v -X POST -H "Content-Type: application/json" --data "{login: vasya}" 'localhost:5000/advanced?id=1&name=vasya&surname=petrovich'
